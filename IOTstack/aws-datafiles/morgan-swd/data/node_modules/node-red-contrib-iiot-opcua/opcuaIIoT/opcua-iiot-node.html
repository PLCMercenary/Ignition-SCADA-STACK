...
<input type='text' id='node-input-nodeId' placeholder='ns=2;s=MySwitch or ns=2;i=1234'> </div> <div class='form-row'> 
<label for='node-input-datatype'><i class='icon-search'></i> <span data-i18n='opcua-iiot-contrib.label.datatype'>
</span></label> <input type='text' id='node-input-datatype' style='width:40%' placeholder='i=2041'> <a 
...
<hr> <div class='form-row'> <label style='min-width:160px' for='node-input-showErrors'><i class='fa fa-th'></i> <span 
data-i18n='opcua-iiot-contrib.label.showErrors'></span></label> <input type='checkbox' id='node-input-showErrors' 
style='max-width:30px'> </div> </script> <script type='text/x-red' data-help-name='OPCUA-IIoT-Node'>
 <h2>OPC UA IIoT Node</h2> <p>
The Node is to set up a flow with the Node-RED Inject node to give the OPC UA Node-Id and the data type parameters.</p>
 <h3>Input</h3> <div> <strong>Event message:</strong> <ul> <li>payload (Value) </li><li>topic (Node-Id) </li></ul> 
</div> <h3>Output</h3> <div> <strong>Event message:</strong> <ul> <li>payload (payload or value) </li><li>
topic (Node-Id) </li><li>datatype (node-opcua/OPC UA data type string) </li><li>nodetype (node) </li></ul> </div> 
