...
.opcuaiiotinject-time-row{padding-left:110px}.opcuaiiotinject-time-row select{margin:3px 0}.opcuaiiotinject-time-days label{-webkit-user-select:none;-khtml-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;vertical-align:top;width:100px}.opcuaiiotinject-time-days input{width:auto}.opcuaiiotinject-time-times{width:90px}#opcuaiiotinject-time-time{width:75px}.opcuaiiotinject-time-count{width:40px!important}
    ...
    align: 'left',
    icon: 'opcuaiiot.png'
  }) </script> <script type='text/javascript'> let injectNode = RED.nodes.registry.getNodeType('OPCUA-IIoT-Inject')
  injectNode.button = {
    onclick: function () {
  ...
  injectNode.oneditprepare = function () {
    ...
    if (node.addressSpaceItems !== undefined && node.addressSpaceItems.length > 0) {
 ...
 <div class='form-row'> <ul style='background:#fff;min-width:600px;margin-bottom:20px' id='node-input-inject-tabs'>
</ul> </div> <div id='node-input-inject-tabs-content' style='min-height:170px'> <div id='opcuaiiotinject-tab-settings' 
style='display:none'> <div class='form-row' id='node-inject-type'> <label for=''><i class='fa fa-repeat'></i> <span 
...
for='node-input-topic'><i class='fa fa-tasks'></i> <span data-i18n='node-red:common.label.topic'></span></label> <input
 type='text' id='node-input-topic'> </div> <div class='form-row'> <label for=''><i class='fa fa-repeat'></i> <span 
...
<option value='30'>30</option> <option value='0'>60</option> </select> <span data-i18n='opcuaiiotinject.minutes'>
</span><br> <span data-i18n='opcuaiiotinject.between'></span> <select id='opcuaiiotinject-time-interval-time-start' 
...
</div> <div> <label><input type='checkbox' checked='checked' value='4'> <span data-i18n='opcuaiiotinject.days.3'>
</span></label> <label><input type='checkbox' checked='checked' value='5'> <span data-i18n='opcuaiiotinject.days.4'>
</span></label> <label><input type='checkbox' checked='checked' value='6'> <span data-i18n='opcuaiiotinject.days.5'>
</span></label> </div> <div> <label><input type='checkbox' checked='checked' value='0'> <span 
data-i18n='opcuaiiotinject.days.6'></span></label> </div> </div> </div> </div> <div class='form-row' id='node-once'> 
<label for='node-input-once' style='width:140px'> <i class='fa fa-power-off'></i> <span 
data-i18n='opcuaiiotinject.onstart'></span></label> <input type='checkbox' id='node-input-once' style='width:40px'> 
</div> <div class='form-row' id='node-delay'> <label for='node-input-startDelay'><i class='fa fa-clock-o'></i> <span 
data-i18n='opcuaiiotinject.startDelay'></span></label> <input type='text' id='node-input-startDelay' placeholder='10' 
style='width:80px'> </div> <div class='form-row'> <label for='node-input-name'><i class='fa fa-tag'></i> <span 
data-i18n='node-red:common.label.name'></span></label> <input type='text' id='node-input-name' 
data-i18n='[placeholder]node-red:common.label.name'> </div> <div class='form-tips' 
...
style='box-sizing:border-box;border-radius:5px;height:580px;padding:5px;border:1px solid #ccc;overflow-y:scroll'> <ol 
...
With the repeat settings you can set a cron configuration.</p> <strong>Inject once at start</strong> <p>
...
<p>Name in the flow of Node-RED.</p> <h3>Tab OPC UA Items</h3> <strong>Name</strong> <p>Name in the flow of Node-RED.
</p> <strong>Node-Id</strong> <p>The Node-Id has to be an OPC UA valid Node-Id like ns=0;i=85 (root->Objects)</p> <p>
You can use our Browser node for OPC UA to find the correct Node-Id.</p> <strong>Data Type</strong> <p>
The Data Type setting is needed to do the converts of the node-opcua Variant type result(s).</p> <p>
In best practice do transformations with the Result Filter node.</p> </script> 